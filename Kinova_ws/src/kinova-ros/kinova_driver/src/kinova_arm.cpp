...
// 获取机械臂末端的z坐标
float KinovaArm::getCartesianZ()
{
    KinovaPose position;
    kinova_comm_.getCartesianPosition(position);
    return position.Z;
}

// 获取机械臂末端的z坐标
float KinovaArm::getCartesianX()
{
    KinovaPose position;
    kinova_comm_.getCartesianPosition(position);
    return position.X;
...
// 获取机械臂末端的z坐标
void KinovaArm::getCartesianPos(KinovaPose& position)
{
    kinova_comm_.getCartesianPosition(position);
}

// 设置手指位置
bool KinovaArm::setFingerPos(float finger1, float finger2, float finger3)
{
    FingerAngles fingers;
    fingers.Finger1 = finger1;
    fingers.Finger2 = finger2;
    fingers.Finger3 = finger3;
    kinova_comm_.setFingerPositions(fingers);
    return true;
}
